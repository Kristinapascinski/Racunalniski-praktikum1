...
\end{frame}
	...
	\begin{frame}{Okolje \texttt{align*}}
		...
		\begin{align*}
			(a+b)^n &= 
			\only<1>{\ldots} % Prikaz na prvi 
			\onslide<2-3>{(a+b)(a+b)\dots(a+b)} \\ % Prikaz na drugi in tretji 
			\onslide<3>{&= a^n + n a^{n-1} b + \dots + \binom{n}{k} a^{n-k} b^k + \dots + n a b^{n-1} + b^n} \\ % Prikaz na tretji
			\onslide<1->{&= \sum_{k=0}^n \binom{n}{k} a^{n-k} b^k.} % Vedno 
		\end{align*}
	\end{frame}
