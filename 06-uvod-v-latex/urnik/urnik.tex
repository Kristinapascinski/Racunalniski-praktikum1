...
% Pri tej naloggi (jjin tudi vseh naslednjih) si pomagajte s paleto ukazov!
...
\[\text{Velikost tabele} = \text{steviloUrNaDan} \cdot \text{steviloDelovnihDni}\] 
...
tip (npr. {\normalsize bool} v C++), kjer bi vrednost {\normalsize true} predstavljala
dosegljivost, {\normalsize false} pa zasedenost.
...
kršitev omejitve n, z upa njenlo utež. Z naslednjo formulo izračunamo
kakovost urnika:kk
...
zato si je smiselno podrobneje ogledati posamezne korake.
   ...
   označimo s ??.Naslednja vgnezdena zanka nas popelje po vseh predavalnicah. Predavalnico na
   ??-tem koraku označimo s ??.Za vse proste termine ?? izračunamo ceno vstavljanja
   ...
   ceno vstavljanja.Ko preverimo vse proste termine vseh predavalnic, vstavimo ??
...
\noindent /Sistem okolic/ je množica vseh možnih sprememb, ki jih lahko z vsemi operacijami iz
...
Rešitev se ponuja kar sama - za naslednjih $x$ korakov prepovemo vsako spremembo, ki jo
opravimo v koraku optimizacije. Izbira konstante $k$ je odvisna od samega problema, saj
ne obstaja neki splošen $k$, ki bi nas v vsakem primeru pripeljal iz lokalnega ekstrema.
